...
class lasot_flow(BaseVideoDataset):
    """ LaSOT dataset.
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, vid_ids=None, split=None, data_fraction=None, min_score = 0.5):
        """
        args:
        ...
        super().__init__('lasot_flow', root, image_loader)
        ...
        print('LaSOT : Optical Flow score: %s, keep ratio: %f'%(self.threshold, L2/L1))
    ...
    def _get_valid_seq(self):
        max_file =  os.path.join(self.flow_root,'lasot_max.txt')
    ...
    def _read_flow_anno(self, seq_id):
        ...
        det_file = os.path.join(self.flow_root,'lasot', seq_name+'.txt')
        try:
    ...
    def _get_sequence_length(self, seq_id): 
        ...
        img_lst  = glob.glob(os.path.join(seq_path, 'img', '*.jpg'))
        return len(img_lst)
    ...
    def get_frames(self, seq_id, frame_ids, anno=None, multi_box = False):
        ...
        anno_frames = {}
        det_gt = self._read_flow_anno(seq_id) # start from 0.
