...
class youtubevos_flow(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, seq_ids=None, data_fraction=None, min_score = 0.5):
        ...
        super().__init__('youtubevos_flow', root, image_loader)
        self.root = os.path.join(self.root, "train_all_frames", "JPEGImages")
        ...
        #self.sequence_list.remove("ef45ce3035") # missing frame
        ...
        print('YouTubeVOS: Optical Flow score: %s, keep ratio: %f'%(self.threshold, L2/L1))


        if data_fraction is not None:
            self.sequence_list = random.sample(self.sequence_list, int(len(self.sequence_list)*data_fraction))
        self.sequence_meta_info = self._load_meta_info()
        self.start_dic = self._get_sequence_start()

    def get_name(self):
        return 'youtubevos_flow'
    ...
    def _get_valid_seq(self):
        max_file =  os.path.join(self.flow_root,'ytvos_max.txt')
    ...
    def _get_sequence_list(self):
        file_path = os.path.join(self.ltr_path, 'data_specs', 'youtubevos_jjtrain.txt')
        dir_list = pandas.read_csv(file_path, header=None).squeeze("columns").values.tolist()
        return dir_list

    def _get_sequence_start(self):
        file_path = os.path.join(self.ltr_path, 'data_specs', 'youtubevos_start.txt')
        start_lst = pandas.read_csv(file_path, delimiter=' ', header=None).values
        start_dic = {v[0]:int(v[1]) for v in start_lst}
        return start_dic

    def _read_flow_anno(self, seq_id):
        seq_name = self.sequence_list[seq_id]
        det_file = os.path.join(self.flow_root,'ytvos', seq_name+'.txt')
        try:
    ...
    def _get_sequence_startid(self, seq_id):
        seq_name = self.sequence_list[seq_id]
        start_id = self.start_dic[seq_name]
        return start_id
    ...
    def get_sequence_info(self, seq_id):
        ...
        start = self._get_sequence_startid(seq_id)
    ...
    def _get_frame_path(self, seq_path, frame_id, start):
        return os.path.join(seq_path, '{:05}.jpg'.format(frame_id + start))    

    def _get_frame(self, seq_path, frame_id, start):
        return self.image_loader(self._get_frame_path(seq_path, frame_id, start))

    def get_frames(self, seq_id, frame_ids, anno=None, multi_box=False):
        start = self._get_sequence_startid(seq_id)
        seq_path = self._get_sequence_path(seq_id)
        obj_meta = self.sequence_meta_info[self.sequence_list[seq_id]]
        frame_list = [self._get_frame(seq_path, f_id, start) for f_id in frame_ids]
        ...
        for f_id in frame_ids:
            L_idx = bisect_left(det_gt[:,0], f_id + start)
            R_idx = bisect_right(det_gt[:,0], f_id + start)
            if R_idx > L_idx:
                if not multi_box:
