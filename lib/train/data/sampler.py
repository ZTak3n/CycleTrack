...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                elif self.frame_sample_mode == 'twoframe':
                    template_frame_ids, search_frame_ids = self.get_frame_ids_twoframe(visible)
                elif self.frame_sample_mode == 'unsupervised':
                    try:
                        template_frame_ids, search_frame_ids = self.get_frame_ids_unsupervised(visible)
                    except:
                        template_frame_ids = [1] * self.num_template_frames
                        search_frame_ids = [1] * self.num_search_frames
                    #print(template_frame_ids, search_frame_ids)
                elif self.frame_sample_mode == 'unsupervised_det':
                    template_frame_ids, search_frame_ids = self.get_frame_ids_unsupervised_det(seq_info_dict["valid"])
                else:
            ...
            try:
                ...
                search_frames, search_anno, meta_obj_test = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict, multi_box = True)
                #print(search_frames[0].shape, search_anno['bbox'][0].shape)
                
                H, W, _ = template_frames[0].shape
                #if self.frame_sample_mode == 'unsupervised':
                #    search_anno['bbox'] =  [torch.zeros(4)] * self.num_search_frames
                #    search_anno['mask'] = [torch.zeros((H, W))] * self.num_search_frames
                    
         
                template_masks = template_anno['mask'] if 'mask' in template_anno else [torch.zeros((H, W))] * self.num_template_frames
                search_masks = search_anno['mask'] if 'mask' in search_anno  else [torch.zeros((H, W))] * len(search_frame_ids)
                #print(dataset.get_name(), template_anno['bbox'])
    ...
    def get_frame_ids_twoframe(self, visible):
        start_ids = 0
        
        if len(visible) <= self.max_gap:
            final_vis_ids = len(visible) - 1
        else:
            final_vis_ids = self.max_gap
        while visible[final_vis_ids] == 0:
            final_vis_ids -= 1
        
        if random.randint(0,1) == 1:  # SWAP
            temp = start_ids
            start_ids = final_vis_ids
            final_vis_ids = temp
        
        template_frame_ids = [start_ids] * self.num_template_frames
        search_frame_ids = [final_vis_ids] * self.num_search_frames
        #print(search_frame_ids, template_frame_ids)
        return template_frame_ids, search_frame_ids
    
    def get_frame_ids_unsupervised(self, visible):
        start_ids = 0
        template_frame_ids = [start_ids] * self.num_template_frames
    ...
    def get_frame_ids_unsupervised_det(self, visible):
        # NOTICE: just one template mode
        
        template_frame_ids = self._sample_visible_ids(visible,
                                                      num_ids=self.num_template_frames)
