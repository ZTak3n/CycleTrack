...
def bbox_to_gauss(bb, sz):
        gauss_scale = 0.25
        #mask = torch.zeros((1,*sz), dtype=torch.float32)
        ...
        dtotsqr = d_xcoords**2/(gauss_scale*w)**2 + d_ycoords**2/(gauss_scale*h)**2
        mask = torch.exp(-0.5*dtotsqr)
        return mask

def resize_sample_target(im, target_bb=None, output_sz=None, mask=None):
    """ Resize the image

    args:
        im - cv image
        target_bb - target box [x1, y1, w, h]
        output_sz - (float) Size to which the extracted crop is resized (always square). If None, no resizing is done.
    ...
    if target_bb is not None:
        if not isinstance(target_bb, list):
            ...
            x, y, w, h = target_bb
        gauss_mask = bbox_to_gauss([x,y,w,h], im.shape[:2]).numpy()
    else:
        gauss_mask = None
    ...
    if output_sz is not None:
        ...
        gauss_mask = cv.resize(gauss_mask, (output_sz, output_sz)) if gauss_mask is not None else None
        if mask is None:
            return im_resized, resize_factor, gauss_mask
        mask_resized = \
        F.interpolate(mask[None, None], (output_sz, output_sz), mode='bilinear', align_corners=False)[0, 0]
        return im_resized, resize_factor, gauss_mask, mask_resized

    else:
        if mask is None:
            return im, gauss_mask, 1.0
        return im, 1.0, gauss_mask, mask
...
def resize(frames, box, output_sz, masks=None):
    ...
    if masks is None:
        crops_resize_factors = [resize_sample_target(f, output_sz= output_sz)
                                for f in frames]
        frames_crop, resize_factors, att_mask = zip(*crops_resize_factors)
        masks_crop = None
    else:
        crops_resize_factors = [resize_sample_target(f, output_sz=output_sz, mask=m)
    ...
    if box is not None:
        ...
        return frames_crop, None, att_mask, masks_crop
