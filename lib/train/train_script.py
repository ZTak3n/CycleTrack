...
# network related
from lib.models.cycletrack import build_cycletrack
# forward propagation related
from lib.train.actors import CycleTrackActor
# for import modules
import importlib


def run(settings):
    settings.description = 'Training script for CycleTrack'
    ...
    # Create network
    if settings.script_name == "cycletrack":
        net = build_cycletrack(cfg)        
    else:
        raise ValueError("illegal script name " + settings.script_name)
    ...
    if settings.script_name == "cycletrack":
        ...
        loss_weight = {'ce': cfg.TRAIN.CE_WEIGHT, 'giou': cfg.TRAIN.GIOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT}
        actor = CycleTrackActor(net = net, objective = objective, loss_weight = loss_weight, settings = settings, cfg = cfg)
    else:
