...
from lib.models.cycletrack.network_utils import Mlp, TransformerDecoder, TransformerDecoderLayer, Box2SeqEmbeddings, generate_square_subsequent_mask
...
class Block(nn.Module):
    ...
    def forward(self, x):
        #print(x.shape) # [bs,temp + search feature,hidden_dim] [bs, 512, 768]
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, search_size=384, template_size=192, 
        ...
        self.box_pos_embed = nn.Parameter(torch.zeros(1, self.num_template, embed_dim))
        self.pos_drop = nn.Dropout(p=drop_rate)
        self.object_token = nn.Parameter(torch.zeros(1, 1, embed_dim))

        self.num_coordinates = 4 # [x,y,w,h]
        #self.word_embed = Box2SeqEmbeddings(bins+2, embed_dim, drop_rate)
        #self.word_embed = MLP(1, 1024, embed_dim, 2)
        self.word_embed = Mlp(4, 1024, embed_dim, drop = drop_rate)
    ...
    def forward_features(self, images_list, seq):
        ...
        c = self.embed_dim_list[0]
        bs = seq.shape[0]
        box_embed = self.word_embed(seq.reshape(-1,4)).view(bs, -1, c)
        box_embed = box_embed + self.box_pos_embed
        object_embed = torch.tile(self.object_token, (bs,1,1))
        #print(box_embed.shape)
        ...
        for i in range(num_search):
            ...
            x = x + self.temporal_pos_embed_x
            x_list.append(x)
        x_feat = torch.cat(x_list, dim=1)
        
        
        xz_feat = torch.cat([x_feat, z_feat, box_embed, object_embed], dim=1)

            
        xz = self.pos_drop(xz_feat)

        #x_feat: [bs, x_feature, hidden_dim] [bs, 256, 768] 
        #xz_feat & xz: [bs, xz_feature, hidden_dim] [bs, 512, 768] [bs, 768, 768]
    ...
    def forward(self, images_list, seq):
        xz = self.forward_features(images_list, seq)
        out=[xz]
        return out
...
def load_pretrained(model, pretrain_type='default', cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True):
    ...
    drop_mae = True
    if drop_mae is True:
        # unsupervised pretrain vit
        state_dict = torch.load('./k700_800E_checkpoint_final.pth')
    else:
